...
static void switch_user(struct passwd *user, char **program, char change_environment __attribute__((unused)))
{
    ...
    if (execvp(*program, program) == -1) {
        (void)write(STDERR_FILENO, *program, strlen(*program));
